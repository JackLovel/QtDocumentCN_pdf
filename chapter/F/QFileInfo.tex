...
\textbf{注意}: 在NTFS文件系统上，出于性能原因，默认情况下禁用所有权和权限检查。
要启用它，请包含以下行：
...
\textbf{另请参阅}: setFile()，isRelative()，QDir::setCurrent() 和
...
\textbf{注意}: 空的 QFileInfo 对象不包含文件引用。
...
\textbf{另请参阅}: dir()，filePath()，fileName() 和 isRelative()
